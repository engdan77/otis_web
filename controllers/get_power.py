...
def timeAboveUnder(argList, argAbove, argUnder):
    # Return how many minutes within range
    # List like [((u'89', u'2014-06-30 21:24:42'),), ((u'100', u'2014-07-01 18:07:15') ....
    
    tot_hours = 0
    if len(argList) > 1:
        # Get first status
        first_element = argList[0]
        if int(first_element[0]) >= argAbove and int(first_element[0]) <= argUnder:
            currently_within = True
            # Add midnight as start time
            last_date = argList[1][1][:10] + " 00:00:01"
        else:
            currently_within = False
        del argList[0]
        # Check of within in range last state, then adjust midnight at end date
        if argList[-1][0] >= argAbove and argList[-1][0] <= argUnder:
            midnight = argList[1][1][:10] + " 23:59:59"
            argList.extend((0, midnight))
    
    if len(argList) == 1:
        # If already on return 24 hours
        if int(argList[0][0]) >= argAbove and int(argList[0][0]) <= argUnder:
            return 24
        else:
            return 0
    ...
    for row in argList:
        value = int(row[0])
        date = row[1]
        if currently_within and (value < argAbove or value > argUnder) :
            tot_hours += diffdates(last_date, date)
            # print "adding : " + str(diffdates(last_date, date))
            currently_within = False
        if not currently_within and (value >= argAbove and value <= argUnder):
            currently_within = True
            last_date = date
    return tot_hours
...
def power_status_period(listDates, argDevList, currentDate=None):
    ...
    for element in argDevList:
        ...
        if currentDate is None:
            ...
            for date in listDates:
                rows = []
                # Get all power change rows for that device
                ...
                # Check all rows and check for how long time power been above threshold
                ...
                result[name] = [ int(x[0]) for x in rows ]
    return result
...
def index():
    ...
    if len(request.vars) > 0:
        ...
        if len(dateRange) == 1:
            ...
            statusList = power_status_period(dateRange, devList, startDate)
        else:
            statusList = power_status_period(dateRange, devList)
        # response.headers['Content-Type']='image/svg+xml'
        # chart_power = pygal.Line(interpolate='hermite', interpolation_parameters={'type': 'cardinal', 'c': .75})
        
        chart_power = pygal.DateY()
        chart_power.show_y_guides = False
        chart_power.show_dots = False
        
        # Change display format depending of dateformat, time of day, or date
        if (re.search(r'\d{2}:\d{2}:\d{2}', dateRange[0])):
            chart_power.x_label_format = "%H:%M"
        else:    
            chart_power.x_label_format = "%Y-%m-%d %H:%M"

        
        # chart_power.show_x_guides = False
        # chart_power.y_labels = False
        #chart_power.show_minor_y_labels = False
        #chart_power = pygal.Line()
        # gen_line_graph(chart_power, dateRange, statusList)
        
        gen_datey_graph(chart_power, dateRange, statusList)
        return chart_power.render()
    else:
        motion_select = get_all_sensormatch_at_location(POWERTYPE, True)
